...
<div class="upper flex justify-between mb-4">
    <span class="text-gray text-2xl salsa ">Categories</span>
            ...
            <?php 
                ...
                if (count($items) > 0){
                    ...
                    <tr class="border-color" data-id="<?= $item['id']; ?>">
                        <td class="text-gray text-medium text-sm p-3 py-4 whitespace-nowrap"><?= ucwords($item['category_name']); ?></td>
...
<div class="py-20 px-4 transition duration-150 ease-in-out z-10 fixed top-0 right-0 bottom-0 left-0 hidden h-full" id="add-modal">
    ...
    <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-xl">
        <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
            <h1 class="text-gray-800 font-lg font-medium tracking-normal leading-tight mb-4">Add category</h1>
            <form id="add_category" action="add_category.php" method="POST" enctype="multipart/form-data">
...
function submitForm(event) {
    ...
    .then(data => {
        ...
        if (data.insert === true) {
            ...
            newRow.innerHTML = `
                <td class="text-gray text-medium text-sm p-3 py-4 whitespace-nowrap">${data.category_name}</td>
                <td class="text-gray text-medium text-sm p-3 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-4">
    ...
    confirmDeleteBtn.addEventListener("click", () => {
        ...
        fetch(`delete_category.php?id=${itemId}`, {
            method: "DELETE",
        })
        .then(response => {
            if (!response.ok) {
