            ...
            <?php 
                ...
                if (count($items) > 0){
                    ...
                    <tr class="border-color" data-id="<?= $item['id']; ?>">
                        ...
                        <td class="text-gray text-medium text-sm p-3 py-4 whitespace-nowrap"><?= ucwords($item['description']) ? ucwords($item['description']) : 'N/A' ?></td>
                        <td class="text-gray text-medium text-sm p-3 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-4">
                                <button title="Edit" id="editModalBtn" class="w-6 h-6" onclick="showEditModal(<?= $item['id'] ?>)"><img src="../images/edit-svgrepo-com.svg" alt=""></button>
                                <button title="Delete" id="deleteModalBtn" class="w-6 h-6" onclick="showDeleteModal(<?= $item['id'] ?>)"><img src="../images/delete-svgrepo-com.svg" alt=""></button>
                            </div>
                        </td>
                    </tr>   
                <?php
            ...
            <form id="add_item" action="add_item.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="mt-5 grid cols-grid-1 cols-grid-2 gap-x-2">
                    ...
                    <div class="col-span-1">
                        ...
                        <input title="Name" name="name" id="name" class="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-amber-400 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="Cup" type="text" autocomplete="off" required>
                    </div>
                    <div class="col-span-1">
                        <label class="text-gray-800 text-sm font-medium leading-tight tracking-normal salsa" for="quantity">Quantity</label>
                        <input title="Quantity" type="text" name="quantity" id="quantity" class="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-amber-400 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border" placeholder="100/10L/10KG"/>
                    </div>
                    <div class="col-span-full">
                        <label class="text-gray-800 text-sm font-medium leading-tight tracking-normal salsa" for="description">Description</label>
                        <textarea title="Description" name="description" id="description" class="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-amber-600 font-normal w-full flex items-center pl-3 py-2 text-sm border-gray-300 rounded border" rows="3" autocomplete="off" required></textarea>
                    </div>
                </div>
    ...
    <div class="w-full max-w-lg p-5 relative mx-auto h-80 rounded-xl shadow-lg  bg-white ">
        <div class="">
            ...
            <div class="p-3  mt-2 text-center space-x-4 md:block">
                <button title="Delete" class="deleteItem mb-2 md:mb-0 bg-red-500 border border-red-500 px-5 py-2 text-sm shadow-sm font-medium tracking-wider text-white rounded-full hover:shadow-lg hover:bg-red-600" data-id=<?= $item['id']; ?> >Delete</button>
